import { combineLatest, fromEvent, generate, interval, noop, of } from "rxjs";
import { map, mergeMap, pluck, scan, startWith, takeWhile, tap, toArray } from "rxjs/operators";
import { gameSize } from "./constants";
import { render } from "./html-renderer";
import { Ball, GameObject, Player } from "./interfaces";

const createGameObject: (x: number, y: number) => {x: number, y: number} = (x: number, y: number) => ({x, y});
...
const processGameCollisions = (_: any, [player, ball, bricks]: [Player, Ball, GameObject[]]) : [Player, Ball, GameObject[]] => (
